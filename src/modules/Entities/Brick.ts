...
export class Brick {
    ...
    public constructor(width: number, height: number, ctx: CanvasRenderingContext2D, specialBrick: boolean, status: number, brick_x: number, brick_y: number, brickPoints: BrickPoints) {
        this.width = width
        this.height = height
        this.ctx = ctx
    ...
    private setColor<T>(specialBrick: boolean, x: number, y: number, image: T): void {
